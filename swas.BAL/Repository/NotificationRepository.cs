    ...
    public class NotificationRepository : INotificationRepository
    {
        ...
        public async Task<int> GetNotificationCountByType(int Type, int? ToUnitId)
        {
            int notificationCount = await _context.Notification
                .Where(n => n.NotificationType == Type && n.IsRead == false &&  n.NotificationTo == ToUnitId)
                .CountAsync();
        ...
        public async Task<Notification> GetNotifByToUnitAndType (int type, int? toUnitId, int projId)
        {
            var notification = await _context.Notification
                .FirstOrDefaultAsync(n => n.NotificationType == type && n.IsRead == false && n.NotificationTo == toUnitId && n.ProjId == projId);
        ...
        {  
            var notifications = await _context.Notification
                .Where(n => n.NotificationTo != unitId  &&  n.ProjId == projId)
                .ToListAsync();
